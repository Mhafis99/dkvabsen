...
export default function GuruAbsensiPage() {
  ...
  const fetchAbsensi = async () => {
    try {
      const response = await fetch(`/api/absensi?guruId=${session?.user?.id}`);
    ...
    <DashboardLayout role="guru">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Riwayat kehadiran bimbingan</p>
        </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="mt-2 text-sm text-muted-foreground">
                        <p>Siswa: {absensi.jadwal.siswa.nama} ({absensi.jadwal.siswa.kelas.namaKelas})</p>
